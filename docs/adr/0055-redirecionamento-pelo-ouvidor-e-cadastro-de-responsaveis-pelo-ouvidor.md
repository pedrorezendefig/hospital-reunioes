...
- A tela `/ouvidoria/responsaveis` e as rotas de escrita de `/responsaveis` passam a aceitar o [Perfil da Ouvidoria] inteiro. Quando esta ADR foi escrita, ela dizia que o histórico de vigência já registrava quem mexeu; não registrava, e a linha do cadastro sozinha não responde mais "quem trocou o titular" agora que a porta tem dois donos. O rastro entrou na issue #711 pelo `audit_log`, a mesma tabela dos outros atos administrativos do app, com o setor e o papel no `metadata`. Rejeitada a coluna de autor na própria tabela de responsáveis: custa uma migration aplicada à mão em produção para guardar o que a auditoria do app já sabe guardar.
