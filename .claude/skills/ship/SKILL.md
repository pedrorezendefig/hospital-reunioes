...
- Vincular ao PRD: sub-issue nativa (`gh api .../sub_issues`) ou "Pai: #N" no corpo.
...
Essas issues alimentam a aba **Pendências** do painel (`tools/workflow-dashboard`), que lê a fila `ready-for-human` direto do GitHub — nenhum arquivo de estado paralelo. Elas ficam fora das ondas do Plano (fila humana, não de agente); ao concluir, o humano fecha a issue e a pendência some do painel.
