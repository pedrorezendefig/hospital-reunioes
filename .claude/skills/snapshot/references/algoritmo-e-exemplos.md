...
**Exit 4, o vizinho do 3.** Quando a introspecção **não consegue rodar** (sem `.venv`, sem `.env`, import quebrado), o fallback AST é legítimo para gerar do zero, mas por cima de um `ROTAS.md` que veio da introspecção ele subtrai: perde as rotas criadas por factory e, pior, não enxerga as dependências dos routers e vira a coluna Auth de ✅ para ❌ em rota autenticada. O banner de listagem parcial fala de rota que falta, não de auth que mente, e ninguém lê banner num commit automático. Nesse caso o script **pula** o `ROTAS.md` e o bloco `AUTO:rotas` do `ARQUITETURA.md`, gera os outros quatro arquivos, avisa no stderr e sai com 4. A flag `--aceitar-listagem-parcial` libera o `ROTAS.md` (que pelo menos sai carimbado), nunca o bloco do `ARQUITETURA.md`. Cobertura em `test_snapshot_nao_rebaixa_rotas.py`.
