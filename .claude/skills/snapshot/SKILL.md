...
**Piso de sanidade (`PISO_ROTAS`).** Se a enumeração vier abaixo do piso, o helper sai com código 2 e o `snapshot.py` **interrompe a geração** (`EnumeracaoDeRotasQuebrada`, exit 3) em vez de cair no fallback AST. Uma varredura que não acha nada satisfaz qualquer asserção sem olhar rota nenhuma, e um `ROTAS.md` mutilado não avisa ninguém. Isso é diferente de "não consegui montar o app", onde o fallback abaixo continua valendo. Cobertura em `hospital-reunioes/backend/tests/test_snapshot_enumeracao_rotas.py`.
