...
def gerar_pdf_pop(*, pop: dict, setor: dict, versao: dict, nomes_designados: dict[str, str | None]) -> bytes:
    ...
    # Fatia 2; aqui cada seção entra como conteúdo. A seção de fluxograma (ADR
    # 0017) embute o SVG capturado no cliente (campo `svg`); sem SVG (ainda não
    # capturado ou Mermaid inválido), o template cai no texto bruto.
