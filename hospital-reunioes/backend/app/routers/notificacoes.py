...
async def list_notificacoes(
    ...
    query = supabase.table("notificacoes").select("*").eq(
        "destinatario_id", participante_id
    )
...
async def count_notificacoes(
    ...
    participante_id = participante["id"]
    result = supabase.table("notificacoes").select(
        "id", count="exact"
    ).eq("destinatario_id", participante_id).eq("lida", False).execute()
...
async def marcar_lida(
    ...
    result = supabase.table("notificacoes").update(
        {"lida": True}
    ).eq("id", notificacao_id).eq("destinatario_id", participante_id).execute()
