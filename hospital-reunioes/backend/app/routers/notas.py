...
async def transcrever_nota(
    audio: UploadFile = File(...),
    current_user: dict = Depends(get_current_user),
    supabase=Depends(get_supabase_client),
):
    """Comando por voz da Nota (issue #35): recebe o áudio ditado e devolve o
    texto transcrito para cair **editável no corpo** — a criação da Nota segue
    sendo um salvar explícito à parte.
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail="Não foi possível transcrever o áudio agora. Digite a nota manualmente.",
        ) from e
    logger.info(f"Nota transcrita por voz para {me['id']}: {len(texto)} chars")
    return {"texto": texto}
