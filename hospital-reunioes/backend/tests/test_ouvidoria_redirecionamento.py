...
class TestOEstouroConsumadoDaAreaAntiga:
    ...
    def test_redirecionar_para_a_mesma_area_nao_limpa_a_ficha_dela(self, monkeypatch, _nunca_envia_email_de_verdade):
        """O avesso, e é regra: zerar o carimbo na área RECONFIRMADA
        transformaria o redirecionamento num jeito de a área limpar a própria
        ficha, que é o que a migration 076 existe para impedir."""
        client, supabase = _com_o_caso_na_recepcao(monkeypatch, _nunca_envia_email_de_verdade)
        _caso(supabase)["prazo_area_em"] = "2026-08-24T20:00:00+00:00"
        ...
        assert resposta.status_code == 201, resposta.text
        assert _caso(supabase)["area_estourou_em"] == "2026-08-24T20:00:00+00:00"
...
class TestValidarTransicaoExigeOMotivo:
    ...
    def test_a_transicao_generica_nao_vira_redirecionamento_sem_observacao(
        self, monkeypatch, _nunca_envia_email_de_verdade
    ):
        """A porta de fundo que o ADR 0055 nomeia, pela ROTA: ela leva o caso a
        `em_classificacao` sem parar o relógio, sem derrubar os links e sem
        acionar ninguém. Continuar aceitando-a é deliberado, mas só dizendo por
        quê."""
        client, supabase = _com_o_caso_na_recepcao(monkeypatch, _nunca_envia_email_de_verdade)

        muda = client.post("/api/ouvidoria/manifestacoes/uuid-7/transicoes", json={"estado": "em_classificacao"})

        assert muda.status_code == 422, muda.text
        assert _caso(supabase)["status"] == "aguardando_area"

        com_motivo = client.post(
            "/api/ouvidoria/manifestacoes/uuid-7/transicoes",
            json={"estado": "em_classificacao", "observacao": MOTIVO},
        )

        assert com_motivo.status_code == 200, com_motivo.text
...
class TestAMigration:
    ...
    def test_a_migration_e_a_mais_recente_a_recriar_o_grafo(self):
        """Duas migrations desta leva recriando o grafo, e a de número maior
        sem uma aresta, deixariam o banco com o corpo errado. `CREATE OR
        REPLACE` não avisa: o último a rodar é o que vale."""
        recriam = sorted(
