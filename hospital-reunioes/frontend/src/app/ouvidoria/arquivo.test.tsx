 ...
 */
function montar(viva: ReturnType<typeof caso>[], arquivada: ReturnType<typeof caso>[] = []) {
  chamadas = [];
  const vivos = [...viva];
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (url: string, init?: RequestInit) => {
      ...
      const querArquivados = endereco.includes("arquivados=true");
      return {
        ok: true,
        status: 200,
...
describe("o filtro Arquivados nasce desligado (ADR 0047, decisão 4)", () => {
  it("a primeira carga pede a lista de trabalho, sem o parâmetro do arquivo", async () => {
    ...
    expect(listagens()[0]).not.toContain("arquivados=true");
  ...
  it("ligar o filtro pede a outra lista e troca o que está na tela", async () => {
    ...
    expect(listagens().at(-1)).toContain("arquivados=true");
  ...
  it("desligar o filtro volta para a lista de trabalho", async () => {
    ...
    await linhaDe("2026-0007");
    expect(listagens().at(-1)).not.toContain("arquivados=true");
    expect(screen.queryByText("2026-0008")).toBeNull();
  });
